...
def test_model_transform():
    ...
    assert X_transform.iloc[0]['A'] == 0.75 #veryfing if this affirmation is true
...
def test_model_save():
    ...
    if os.path.isfile(filename): #veryfing if there is any file, for tests, if exists remove it
        os.remove(filename)
